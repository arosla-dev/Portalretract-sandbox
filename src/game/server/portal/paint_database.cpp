...
void CPaintDatabase::PaintEntity( CBaseEntity *pPaintedEntity, PaintPowerType newPowerType, const Vector &vecPosition )
{
	...
	//else // RETRACT: Set all render colors
	{
		...
		else if ( newPowerType != NO_POWER ) // NO_PAINT doesn't use a screen effect in Portal 2
		{
			color32 fadeColor = {newColor.r(),newColor.g(),newColor.b(),32};
			UTIL_ScreenFade( pPaintedEntity, fadeColor, 0.8, 0, FFADE_IN );
		}
	}
}
...
{
#if 0 // No wall painting in retract
...
void CPaintDatabase::PreClientUpdate()
{
	...
	for ( int i=0; i<count; ++i )
	{
		...
		float flChangedPaintRadius = UTIL_PaintBrushEntity( pPaintBrush, data.location, data.type, data.flPaintRadius, data.flPaintAlphaPercent, data.normal );
