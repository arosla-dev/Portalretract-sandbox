...
class CFuncPaintBombControlVolume : public CBaseEntity
{
public:
	DECLARE_CLASS( CFuncPaintBombControlVolume, CBaseEntity );
	DECLARE_DATADESC();
	CFuncPaintBombControlVolume();

	void Spawn();

	float m_flForcedRadius;
};

BEGIN_DATADESC( CFuncPaintBombControlVolume )

	DEFINE_KEYFIELD( m_flForcedRadius, FIELD_FLOAT, "ForcedRadius" ),

END_DATADESC()

LINK_ENTITY_TO_CLASS( func_paintbomb_control_volume, CFuncPaintBombControlVolume );

CFuncPaintBombControlVolume::CFuncPaintBombControlVolume()
{
	m_flForcedRadius = -1;
}

void CFuncPaintBombControlVolume::Spawn()
{
	BaseClass::Spawn();
...
void CreatePaintBombExplosion( PaintPowerType paintType, const Vector& vecExplosionPos, bool bSpawnBlobs )
{
	...
	CFuncPaintBombControlVolume *pControl = NULL;
	CBaseEntity *ent = NULL;
	while ( ( ent = gEntList.FindEntityByClassname( ent, "func_paintbomb_control_volume" ) ) != NULL )
	{
		Vector mins, maxs;
		mins = ent->WorldAlignMins();
		maxs = ent->WorldAlignMaxs();
		if ( IsPointInBox( vecExplosionPos, mins, maxs ) && ((CFuncPaintBombControlVolume*)ent)->m_flForcedRadius != -1 )
		{
			pControl = (CFuncPaintBombControlVolume*)ent;
			break;
		}
	}

	const float flBombRadius = pControl ? pControl->m_flForcedRadius : paintbomb_explosion_radius.GetFloat();
	...
	for( int i = 0; i < dirs.Count(); ++i )
	{
		...
		if ( tr.fraction < flCleanserFraction && tr.m_pEnt != NULL && pPortal == NULL && !((tr.contents & CONTENTS_WINDOW) || (tr.contents & CONTENTS_GRATE))  )
#else
