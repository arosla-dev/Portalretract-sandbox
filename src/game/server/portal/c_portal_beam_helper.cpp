...
C_Beam *C_PortalBeamHelper::CreateBeam( void )
{
	...
	pNewBeam->SetRenderColorA( m_pBeamTemplate->GetRenderColorA() );
...
void C_PortalBeamHelper::UpdatePoints( Vector &vStartPoint, Vector &vEndPoint, unsigned int fMask, ITraceFilter *pTraceFilter, trace_t &tr )
{
	...
	for (nLastCreatedBeam = 0; (bHitPortal) && nLastCreatedBeam < 10 ; ++nLastCreatedBeam)
	{
#ifdef DEBUG
		++iCycleCount2;
#endif
		...
		C_Prop_Portal *pPortal; // [esp+14Ch] [ebp-1Ch] BYREF
