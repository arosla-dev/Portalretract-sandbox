	...
	if ( info.GetAttacker() != this && !(bitsDamage & (DMG_DROWN | DMG_FALL)) ) 
	{
		...
		if ( bitsDamage & DMG_CRITICAL )
		{
			...
			if ( info.GetAttacker() && info.GetAttacker()->IsPlayer() && !m_Shared.InCond( TF_COND_DISGUISED ) )
			{
				...
				DispatchEffect( filter, 0.0, "ParticleEffect", data );
...
bool CTFPlayer::GetResponseSceneFromConcept( int iConcept, char *chSceneBuffer, int numSceneBufferBytes )
{
	...
	if ( result )
	{
		...
		response.GetResponse( chSceneBuffer, numSceneBufferBytes );
...
void CC_PlayerParticleEffect( const CCommand &args )
{
	if ( !args[1] || !strcmp( args[1], "" ) || args[1][0] == '\0' )
	{
		Msg("Usage: PlayParticleEffect < Particle Name >\n");
	}

	CBasePlayer *pPlayer = UTIL_GetCommandClient();

	DispatchParticleEffect( args[1], pPlayer->GetAbsOrigin(), pPlayer->GetAbsAngles() );

}
ConCommand PlayParticleEffect( "PlayParticleEffect", CC_PlayerParticleEffect, "Plays a named particle", FCVAR_CHEAT );