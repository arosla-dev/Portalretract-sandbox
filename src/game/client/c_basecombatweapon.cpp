...
void C_BaseCombatWeapon::OnDataChanged( DataUpdateType_t updateType )
{
		...
		/*|| GetClientAlphaProperty() */)
	{
		...
		UTIL_Remove( m_pWorldModelClone ); // Best Guess!!
...
{	
	if (!m_iWorldModelIndex || (GetEffects() & EF_NODRAW) != 0)
		return false;
...
void C_CombatWeaponClone::UpdateClone( void )
{
	...
	if (m_nLastUpdatedWorldModelClone != iTickCount)
	{
		...
		SetParent( m_pWeaponParent->GetMoveParent(), m_pWeaponParent->GetParentAttachment() );
		...
		//if ( GetModel() != m_pWeaponParent->GetModel() )
		//{
		//	SetModelPointer( m_pWeaponParent->GetModel() );
		//}
#if 1
		CStudioHdr *pStudio = GetModelPtr();
		if ( pStudio
			&& pStudio->GetRenderHdr()
			&& ( m_pWeaponParent->GetSequence() >= pStudio->GetNumSeq() ) )
#else
		if ( false )
#endif
		{
