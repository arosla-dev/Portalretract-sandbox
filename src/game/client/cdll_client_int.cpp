...
bool CL_ShouldLoadBackgroundLevel( const CCommand &args )
{
	...
	if ( args.ArgC() == 2 )
	{
		...
		if ( !Q_stricmp( args[1], "playendgamevid" ) )
		{
			...
			engine->ExecuteClientCmd( "_restart\n" );
			return false;
		}
...
{
#ifdef PORTAL
	// RETRACT_PART2: It's super odd that config.cfg from the mod directly won't execute
	engine->ExecuteClientCmd( "exec config.cfg" );
#endif

	COM_TimestampedLog( "IGameSystem::PostInitAllSystems - Start" );
	IGameSystem::PostInitAllSystems();
	COM_TimestampedLog( "IGameSystem::PostInitAllSystems - Finish" );
#ifdef PORTAL // RETRACT: Start the background map
	CCommand args;
	if ( CL_ShouldLoadBackgroundLevel( args ) )
	{
		char szBackgroundMap[_MAX_PATH];
		engine->GetMainMenuBackgroundName( szBackgroundMap, sizeof( szBackgroundMap ) );

		char szCommand[_MAX_PATH];
		V_snprintf( szCommand, sizeof( szCommand ), "map_background %s\n", szBackgroundMap );
