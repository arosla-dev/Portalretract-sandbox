...
#include "new/entities/game_timer.h"
...
void CHudGenericGameTimer::OnThink()
{
	...
	if (!pTimer || pTimer->IsDisabled() || pTimer->IsMarkedForDeletion())
	{
		if (m_flShutoffTime == 0.0f)
		{
			...
			GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("GenericGameTimerClose");
		}
		m_iTimerIndex = 0;
		return;
	}
	...
	if (m_bTimerPaused && !pTimer->IsTimerPaused())
	{
		...
		GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("GenericGameTimerPulse");
	}
	else if (pTimer->IsTimerPaused())
	{
		m_bTimerPaused = true;
		GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("GenericGameTimerPulseOnce");
	}
	...
	if (flWarnTime > 0.0f)
	{
		if (m_bTimerWarned && flTimeRemaining > flWarnTime)
		{
			...
			GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("GenericGameTimerUnwarn");
		}
		else if (flTimeRemaining <= flWarnTime)
		{
			// Turn red
			m_bTimerWarned = true;
			GetClientMode()->GetViewportAnimationController()->StartAnimationSequence("GenericGameTimerWarn");
		}
	}
...
bool CHudGenericGameTimer::FindTimer()
{
	...
	if (pTimer)
	{
		...
		m_bTimerWarned = false;
		GetClientMode()->GetViewportAnimationController()->StartAnimationSequence(m_bTimerPaused ? "GenericGameTimerShow" : "GenericGameTimerShowFlash");
