	...
	void SetBonusMapInfo( const char *pCurrentPath, BonusMapDescription_t &map )
	{
		...
		if ( map.bIsFolder )
		{
			if ( map.szImageName[ 0 ] == '\0' )
			{
				...
				{
					V_strcpy( szImageFileName, "bonusmaps/icon_bonus_map_folder" );
					bIsTGA = false;
				}
			}
			else
		...
		else
		{
			if ( map.szImageName[ 0 ] == '\0' )
			{
				...
				if( !g_pFullFileSystem->FileExists( szImageFileName, "MOD" ) )
				{
					V_strcpy( szImageFileName, "bonusmaps/icon_bonus_map_default" );
					bIsTGA = false;
				}
			}
			else
...
void CBonusMapsDialog::ApplySchemeSettings( IScheme *pScheme )
{
	...
	// This is all necessary to fix the images from staying when clicking back after clicking on an image
	ipanel()->SetMouseInputEnabled( FindChildByName( "ChallengeMedalOverview00" )->GetVPanel(), false );
	ipanel()->SetMouseInputEnabled( FindChildByName( "ChallengeMedalOverview01" )->GetVPanel(), false );
	ipanel()->SetMouseInputEnabled( FindChildByName( "ChallengeMedalOverview02" )->GetVPanel(), false );
	ipanel()->SetMouseInputEnabled( FindChildByName( "ChallengeMedalOverview03" )->GetVPanel(), false );
	ipanel()->SetMouseInputEnabled( FindChildByName( "ChallengeMedalOverview04" )->GetVPanel(), false );
...
void CBonusMapsDialog::PerformLayout( void )
{
	BaseClass::PerformLayout();
	vgui::ivgui()->AddTickSignal( GetVPanel(), 25 );
}

void CBonusMapsDialog::OnTick( void )
{
	// This is such a horrible fix, but it's the only thing that I know of that will fix it
