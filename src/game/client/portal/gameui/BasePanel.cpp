...
{	
	vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx( NULL, "resource/MenuScheme.res", "MenuScheme"); // RETRACT_PART2
	SetScheme( scheme );
	g_pBasePanel = this;
...
void CBasePanel::RunMenuCommand(const char *command)
{
    ...
    // HPE_END
    //=============================================================================
	else if ( !Q_stricmp( command, "MainMenu" ) )
	{
		OnOpenMainMenuConfirmationDialog();
	}
...
void CAsyncCtxOnDeviceAttached::ExecuteAsync()
{
	...
	// Open user settings and save game container here
	m_ContainerOpenResult = engine->OnStorageDeviceAttached( -1 );
...
public:
	...
	virtual void OnCommand(const char *command)
	{
		if (!Q_stricmp(command, "Quit"))
		{
			//if ( m_bExitToMenu )
			//{
			//	PostMessage(GetVParent(), new KeyValues("Command", "command", "engine disconnect"));
			//	Close();
			//}
			//else
			{
...
//-----------------------------------------------------------------------------
void CBasePanel::OnOpenMainMenuConfirmationDialog()
{
	if ( GameUI().IsConsoleUI() )
	{
	...
	//	CSaveBeforeQuitQueryDialog *box = new CSaveBeforeQuitQueryDialog(this, "SaveBeforeQuitQueryDialog", true);
	//	box->DoModal();
	//}
	//else
	{
		// simple ok/cancel prompt
		QueryBox *box = new CQuitQueryBox("#GameUI_MainMenuConfirmationTitle", "#GameUI_MainMenuConfirmationText", this);
		box->SetOKButtonText("#GameUI_ExitLobby");
		box->SetOKCommand(new KeyValues("Command", "command", "engine disconnect"));
...
// Purpose: simple querybox that accepts escape
//-----------------------------------------------------------------------------
class CPaintWarningDialog: public vgui::Frame
{
	DECLARE_CLASS_SIMPLE( CPaintWarningDialog, vgui::Frame );
public:
	CPaintWarningDialog(vgui::Panel *parent, const char *name) : BaseClass(parent, name)
	{
		LoadControlSettings("resource/PaintWarningDialog.res");
		SetDeleteSelfOnClose(true);
		SetSizeable(false);
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBasePanel::OnOpenPaintWarningDialog()
{
	if ( !m_hPaintWarningDialog )
		m_hPaintWarningDialog = new CPaintWarningDialog( this, "PaintWarningDialog" );

	m_hPaintWarningDialog->Activate();
}
