...
//-----------------------------------------------------------------------------
class COptionsSubVideoAdvancedWarningDlg : public vgui::Frame
{
	DECLARE_CLASS_SIMPLE( COptionsSubVideoAdvancedWarningDlg, vgui::Frame );
public:
	COptionsSubVideoAdvancedWarningDlg( vgui::Panel *parent ) : BaseClass( parent , "OptionsAdvancedWarningDlg" )
	{
		SetTitle("#portalretract_gameui_warning", true);
		SetSize( 400, 100 );
		
		LoadControlSettings( "resource/OptionsSubVideoAdvancedWarningDlg.res" );
		MoveToCenterOfScreen();
		SetSizeable( false );
	}
...
public:
	COptionsSubVideoAdvancedDlg( vgui::Panel *parent ) : BaseClass( parent , "OptionsSubVideoAdvancedDlg" )
	{
		...
		// In Retract, paint doesn't render below high settings
		...
		//m_pColorCorrection->SetEnabled( true );  // RETRACT: setting this to true may not make it save properly
		//m_pMotionBlur->SetEnabled( true );  // RETRACT: setting this to true may not make it save properly
	...
	void MarkDefaultSettingsAsRecommended()
	{
		...
		// FIXME:
		SetComboItemAsRecommended( m_pTextureDetail, clamp( nSkipLevels-2, 0, 1 ) );
	...
	virtual void ApplyChanges()
	{
		...
		ApplyChangesToConVar( "mat_picmip", 0 - m_pTextureDetail->GetActiveItem());
	...
	virtual void OnResetData()
	{
		...
		m_pTextureDetail->ActivateItem( 0 - clamp(mat_picmip.GetInt(), -1, 0) );
...
private:
	bool m_bUseChanges;
	vgui::ComboBox *m_pModelDetail, *m_pTextureDetail, *m_pAntialiasingMode;
	vgui::ComboBox *m_pShadowDetail, *m_pWaterDetail, *m_pVSync, *m_pMulticore;
...
COptionsSubVideo::~COptionsSubVideo()
{
	...
	if (m_hOptionsSubVideoAdvancedWarningDlg.Get())
	{
		m_hOptionsSubVideoAdvancedWarningDlg->MarkForDeletion();
	}
}
...
{
	if ( HASPAINTMAP && strcmp( "", engine->GetLevelNameShort() ) != 0 )
	{
		if ( !m_hOptionsSubVideoAdvancedWarningDlg.Get() )
		{
			m_hOptionsSubVideoAdvancedWarningDlg = new COptionsSubVideoAdvancedWarningDlg( BasePanel()->FindChildByName( "OptionsDialog" ) ); // we'll parent this to the OptionsDialog directly
		}
		
		m_hOptionsSubVideoAdvancedWarningDlg->Activate();
		return;
	}
