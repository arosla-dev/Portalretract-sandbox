...
void C_PaintStream::DrawBlobs( IMaterial* pMaterial )
{
	...
	if ( bListenServer )
	{
		g_SurfaceRenderParticles.EnsureCount( nBlobCount );
		...
		for ( int i = 0; i < nBlobCount; ++i )
		{
			...
			{
				ImpParticle* imp_particle = &(g_SurfaceRenderParticles[ iNumParticles ]);
	...
	else
	{
		...
		g_SurfaceRenderParticles.EnsureCount( 2 * nBlobCount );
