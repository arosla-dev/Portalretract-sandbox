...
{
	if( HASPAINTMAP && !((pEvent->surfaceProps[!index] & CONTENTS_WINDOW) || (pEvent->surfaceProps[!index] & CONTENTS_GRATE)) )
	{
...
void PropPaintPowerUser<BasePropType>::UpdatePaintPowersFromContacts()
{
	...
	{
		// RETRACT_PART2
		// FIXME: Do NOT add surface contacts for CONTENTS_WINDOW AND CONTENTS_GRATE
