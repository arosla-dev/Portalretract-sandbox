...
// RETRACT: Changing bounce_paint_min_speed from 500 to 440 to account for the level design
ConVar bounce_paint_min_speed("bounce_paint_min_speed", "440.0f", FCVAR_REPLICATED | FCVAR_CHEAT, "For tweaking how high bounce paint launches the player.");
// RETRACT: Changing sv_wall_bounce_trade from 0.73 to 0.82 to fix wall jumps
ConVar sv_wall_bounce_trade("sv_wall_bounce_trade", "0.82", FCVAR_REPLICATED | FCVAR_CHEAT, "How much outward velocity is traded for upward velocity on wall bounces");
...
float ComputeHullWidth( const Vector& hullMins, const Vector& hullMaxs )
{
	...
	diffInPlane -= DotProduct( Vector( 0, 0, 1 ), diffInPlane ) * Vector( 0, 0, 1 );
...
// Transform a plane
//-----------------------------------------------------------------------------
inline void MatrixTransformVector4D( const matrix3x4_t &src, const Vector4D &inPlane, Vector4D &outPlane )
{
	...
	VectorRotate( inPlane.AsVector3D(), src, outPlane.AsVector3D() );
	outPlane.w = inPlane.w * DotProduct( outPlane.AsVector3D(), outPlane.AsVector3D() );
	outPlane.w += outPlane.x * src[0][3] + outPlane.y * src[1][3] + outPlane.z * src[2][3];
}

void ComputeAABBContactsWithBrushEntity( CPortal_Player *pPlayer, ContactVector& contacts, const Vector& boxOrigin, const Vector& boxMin, const Vector& boxMax, CBaseEntity* pBrushEntity, int contentsMask )
{
	ComputeAABBContactsWithBrushEntity( pPlayer, contacts, NULL, 0, boxOrigin, boxMin, boxMax, pBrushEntity, contentsMask );
}
...
void ComputeAABBContactsWithBrushEntity_Old( ContactVector& contacts, const cplane_t *pClipPlanes, int iClipPlaneCount, const Vector& boxOrigin, const Vector& boxMin, const Vector& boxMax, CBaseEntity* pBrushEntity, int contentsMask )
{
	...
	for( int i = 0; i < WorldBrushes.Count(); ++i )
	{
		...
		{
			Vector4D temp;
			MatrixTransformVector4D( entityToWorld, brushSides[sideIndex].plane, temp );
			planes[sideIndex] = Vector4D( temp.x, temp.y, temp.z, temp.w );
		}
...
void ComputeAABBContactsWithBrushEntity(  CPortal_Player *pPlayer, ContactVector& contacts, const cplane_t *pClipPlanes, int iClipPlaneCount, const Vector& boxOrigin, const Vector& boxMin, const Vector& boxMax, CBaseEntity* pBrushEntity, int contentsMask)
{
	...
	if ( !pBrushEntity->IsWorld() )
	{
		ComputeAABBContactsWithNonWorldBrushEntity( pPlayer, contacts, pBrushEntity, contentsMask );
	}
}

void ComputeAABBContactsWithNonWorldBrushEntity( CPortal_Player *pPlayer, ContactVector& contacts, CBaseEntity* pBrushEntity, int contentsMask )
{
	//contentsMask = MASK_SOLID;
	const Vector &vecStartPos = pPlayer->WorldSpaceCenter();
	const Vector &vecPlayerMins = pPlayer->GetPlayerMins();
	const Vector &vecPlayerMaxs = pPlayer->GetPlayerMaxs();
	const Vector &vecAbsVelocity = pPlayer->GetAbsVelocity();

	const matrix3x4_t& entityToWorld = pBrushEntity->EntityToWorldTransform();
	// The query box must be in local space for non-world brush entities
		
	const float HALF_HEIGHT = vecPlayerMaxs.z * 0.5;
	const float HALF_WIDTH = 16; // Basically just VEC_HULL_MIN.x, VEC_HULL_MAX.x, VEC_HULL_MIN.y, VEC_HULL_MAX.y

	//Vector vecVelCompensator( 0, 0, 0 );
	trace_t tr;
	const float frametime = gpGlobals->frametime * 2;
	// Downward //
	// Straight down
	float flCompensatorZ = vecAbsVelocity.z * frametime;
	if ( flCompensatorZ > 0.0f )
		flCompensatorZ = 0.0f;
	Vector vecVelCompensator = Vector( 1, 1, flCompensatorZ );
	Vector vecEndPos = vecStartPos + ( Vector( 0, 0, -HALF_HEIGHT ) + vecVelCompensator );
	UTIL_TraceLine( vecStartPos, vecEndPos, contentsMask, pPlayer, COLLISION_GROUP_NONE, &tr );
	if ( tr.m_pEnt == pBrushEntity && !((tr.contents & CONTENTS_WINDOW) || (tr.contents & CONTENTS_GRATE)) )
	{
		goto BRUSH_TRACE_HIT;
	}
	// +x, +y
	float flCompensatorX = vecAbsVelocity.x * frametime;
	if ( flCompensatorX < 0.0f )
		flCompensatorX = 0.0f;
	float flCompensatorY = vecAbsVelocity.y * frametime;
	if ( flCompensatorY < 0.0f )
		flCompensatorY = 0.0f;
	vecVelCompensator = Vector( flCompensatorX, flCompensatorY, flCompensatorZ );
	vecEndPos = vecStartPos + ( Vector( HALF_WIDTH, HALF_WIDTH, -HALF_HEIGHT ) + vecVelCompensator );
	UTIL_ClearTrace( tr );
	UTIL_TraceLine( vecStartPos, vecEndPos, contentsMask, pPlayer, COLLISION_GROUP_NONE, &tr );
	if ( tr.m_pEnt == pBrushEntity && !((tr.contents & CONTENTS_WINDOW) || (tr.contents & CONTENTS_GRATE)) )
	{
	...
	// +x, -y
	flCompensatorY = vecAbsVelocity.y * frametime;
	if ( flCompensatorY > 0.0f )
		flCompensatorY = 0.0f;
	vecVelCompensator = Vector( flCompensatorX, flCompensatorY, flCompensatorZ );
	vecEndPos = vecStartPos + ( Vector( HALF_WIDTH, -HALF_WIDTH, -HALF_HEIGHT ) + vecVelCompensator );
	...
	// -x, +y
	flCompensatorX = vecAbsVelocity.x * frametime;
	if ( flCompensatorX > 0.0f )
		flCompensatorX = 0.0f;
	flCompensatorY = vecAbsVelocity.y * frametime;
	if ( flCompensatorY < 0.0f )
	...
	if ( flCompensatorY > 0.0f )
		flCompensatorY = 0.0f;
	//Msg( "flCompensatorX %f flCompensatorY %f\n", flCompensatorX, flCompensatorY );
	vecVelCompensator = Vector( flCompensatorX, flCompensatorY, flCompensatorZ );
	vecEndPos = vecStartPos + ( Vector( -HALF_WIDTH, -HALF_WIDTH, -HALF_HEIGHT ) + vecVelCompensator );
	UTIL_ClearTrace( tr );
	UTIL_TraceLine( vecStartPos, vecEndPos, contentsMask, pPlayer, COLLISION_GROUP_NONE, &tr );
	...
	// Upward //
	flCompensatorZ = vecAbsVelocity.z * frametime;
	if ( flCompensatorZ < 0.0f )
		flCompensatorZ = 0.0f;
	// +x, +y
	flCompensatorX = vecAbsVelocity.x * frametime;
	...
	// Forward
	UTIL_ClearTrace( tr );
	//UTIL_TraceHull( vecStartPos + Vector( 0, 0, 1 ), vecStartPos - ( Vector( 0, 0, DIM_OFFSET ) ), vecPlayerMins, vecPlayerMaxs, contentsMask, NULL, &tr );
	UTIL_TraceLine( vecStartPos + Vector( 0, 0, 1 ), vecStartPos - ( Vector( 0, 0, DIM_OFFSET ) ), contentsMask, pPlayer, COLLISION_GROUP_NONE, &tr );
	if ( tr.m_pEnt == pBrushEntity )
	{
		goto BRUSH_TRACE_HIT;
	}
	...
	// Backward
	UTIL_ClearTrace( tr );
	//UTIL_TraceHull( vecStartPos + Vector( 0, 0, 1 ), vecStartPos - ( Vector( -DIM_OFFSET, 0, 0 ) ), vecPlayerMins, vecPlayerMaxs, contentsMask, NULL, &tr );
	UTIL_TraceLine( vecStartPos + Vector( 0, 0, 1 ), vecStartPos - ( Vector( -DIM_OFFSET, 0, 0 ) ), contentsMask, pPlayer, COLLISION_GROUP_NONE, &tr );
	...
	// Left
	UTIL_ClearTrace( tr );
	//UTIL_TraceHull( vecStartPos, vecStartPos - ( Vector( 0, DIM_OFFSET, 0 ) ), vecPlayerMins, vecPlayerMaxs, contentsMask, NULL, &tr );
	UTIL_TraceLine( vecStartPos + Vector( 0, 0, 1 ), vecStartPos - ( Vector( 0, DIM_OFFSET, 0 ) ), contentsMask, pPlayer, COLLISION_GROUP_NONE, &tr );
	if ( tr.m_pEnt == pBrushEntity )
	{
		goto BRUSH_TRACE_HIT;
	}
	...
	else
	{
		...
		const char *name = tr.m_pEnt->GetDebugName();
		//Msg( "NO HIT tr.m_pEnt %s\n", name );
		//NDebugOverlay::SweptBox( vecStartPos, vecStartPos - ( Vector( 0, 0, DIM_OFFSET ) ), vecPlayerMins, vecPlayerMaxs, vec3_angle, 0, 255, 0, 64, 0.2 );
		return;
	}
#endif
	BRUSH_TRACE_HIT:

	//Msg( "tr.plane.normal: %f %f %f\n", tr.plane.normal.x, tr.plane.normal.y, tr.plane.normal.z );

	//cplane_t temp;
	//MatrixTransformPlane( entityToWorld, tr.plane, temp );
	//tr.plane = temp;
#if 0
	if ( tr.m_pEnt )
	{
	...
	Msg( "tr.startsolid: %i\n", (int)tr.startsolid );	
	Msg( "tr.plane.normal2: %f %f %f\n", tr.plane.normal.x, tr.plane.normal.y, tr.plane.normal.z );
#endif	
	//NDebugOverlay::SweptBox( tr.startpos, tr.endpos, vecPlayerMins, vecPlayerMaxs, vec3_angle, 255, 0, 0, 64, 0.5 );
		
	//NDebugOverlay::Line( tr.endpos, tr.endpos + (tr.plane.normal * 64 ), 255, 0, 0, true, 5 );

	BrushContact contact;
	contact.Initialize( tr.endpos, tr.plane.normal, pBrushEntity, false );
	contacts.AddToTail( contact );
}
#ifdef GAME_DLL
CON_COMMAND( test_trace, "" )
{
	...
	//UTIL_TraceHull( vecStartPos, vecStartPos - ( Vector( 0, 0, DIM_OFFSET ) ), vecPlayerMins, vecPlayerMaxs, contentsMask, NULL, &tr );
	UTIL_TraceLine( vecStartPos, vecStartPos + ( 1024 * forward ), MASK_ALL, pPlayer, 0, &tr );
	
	Msg( "test trace endpos: %f %f %f\n", tr.endpos.x, tr.endpos.y, tr.endpos.z );
	Msg( "test trace normal: %f %f %f\n", tr.plane.normal.x, tr.plane.normal.y, tr.plane.normal.z );
}
#endif
...
void CPortal_Player::SetLeaveTractorBeam( CTrigger_TractorBeam *pTractorBeam, bool bKeepFloating )
{
	...
	else if ( m_hTractorBeam == NULL )
	{
		...
		{
			CTrigger_TractorBeam *pBeam = static_cast< CTrigger_TractorBeam* >( ITriggerTractorBeamAutoList::AutoList()[ i ]->GetEntity() );
			if ( pBeam == pTractorBeam )
				continue;

			int solidtype = pBeam->GetSolid();
...
void CPortal_Player::SetStepSoundTime( stepsoundtimes_t iStepSoundTime, bool bWalking )
{
	...
	PaintPowerState state = GetPaintPower( SPEED_POWER ).m_State;
	if ( state == ACTIVATING_PAINT_POWER || state == ACTIVE_PAINT_POWER  || state == DEACTIVATING_PAINT_POWER )
	{
		m_flStepSoundTime *= 0.666666f;
	}
}
...
void CPortal_Player::Paint( PaintPowerType type, const Vector& worldContactPt )
{
	if( player_can_use_painted_power.GetBool() || player_paint_effects_enabled.GetBool() )
	{
		...
		m_PaintedPowerType = type;
		m_PaintedPowerTimer.Start( player_paint_effects_duration.GetFloat() );
	}
}
	...
	for( PaintPowerConstIter iter = begin; iter != end; ++iter )
	{
												  ...
												  flVelocityCos ) )
		{
			...
			if( choiceCriteria.bInPortal )
			{
				...
				const bool nearPortal = UTIL_PointIsNearPortal( powerInfo.m_ContactPoint, m_hPortalEnvironment, PORTAL_PLANE_IGNORE_EPSILON, 0.5f * ComputeHullWidth( VEC_HULL_MAX, VEC_HULL_MIN ) - 5.0f );
			...
			// to a speed surface.
			const bool walkingOffStick = 1.0 < STEEP_SLOPE && powerInfo.m_SurfaceNormal.z > STEEP_SLOPE;
...
void CPortal_Player::ChooseActivePaintPowers( PaintPowerInfoVector& activePowers )
{
	...
	if( HasAnySurfacePaintPowerInfo() || LateSuperJumpIsValid() )
	{
		...
		const Vector dvStickGravity = gravityMagnitude * Vector( 0, 0, 1 );
	...
	if( player_can_use_painted_power.GetBool() && m_PaintedPowerType != NO_POWER )
	{
		...
		for( PaintPowerConstIter i = touchedPowerRange.first; i != touchedPowerRange.second; ++i )
		{
			if( i->m_PaintPowerType != INVALID_PAINT_POWER )
			{
				...
				paintedPower.m_PaintPowerType = m_PaintedPowerType;
		...
		if( bestPaintedPowers[m_PaintedPowerType].pPaintPower &&
			!bestPaintedPowers[m_PaintedPowerType].bWasIgnored )
		{
			activePowers.AddToTail( *bestPaintedPowers[m_PaintedPowerType].pPaintPower );
		}
	}
...
void CPortal_Player::AddSurfacePaintPowerInfo( const trace_t& trace, char const* context )
{
	if( trace.m_pEnt )
	{
		if( trace.m_pEnt->IsBSPModel() )
		{
			...
			if ( !(m_nButtons & IN_DUCK) && m_Local.m_bInDuckJump )
			{
				vecBaseMins = VEC_HULL_MIN;
				vecBaseMaxs = VEC_HULL_MAX;
				vecOrigin -= Vector(0, 0, 32);
			}
			else
			{
				vecBaseMins = WorldAlignMins();
				vecBaseMaxs = WorldAlignMaxs();
			}
			//NDebugOverlay::Box( GetAbsOrigin(), vecBaseMins, vecBaseMaxs, 255, 0, 0, 128, 0.03 );

			// Compute the contacts with a slightly expanded swept AABB
			const Vector traceDelta = trace.endpos - trace.startpos;
			const Vector contactBoxMin = vecOrigin + traceDelta + vecBaseMins - AABB_OFFSET,
						 contactBoxMax = vecOrigin + traceDelta + vecBaseMaxs + AABB_OFFSET;
			...
			ComputeAABBContactsWithBrushEntity( this, contacts, pClip, iClipCount, WorldSpaceCenter(), contactBoxMin, contactBoxMax, trace.m_pEnt );
...
void CPortal_Player::DeterminePaintContacts()
{
...
#endif
	//NDebugOverlay::Box( vecOrigin, vecBaseMins, vecBaseMaxs, 255, 0, 0, 128, 0.03 );
	// Compute the contacts with a slightly expanded swept AABB
	Vector contactBoxMin = vecOrigin + vecBaseMins - AABB_OFFSET,
		contactBoxMax = vecOrigin + vecBaseMaxs + AABB_OFFSET;
	const Vector traceBoxMin = contactBoxMin, traceBoxMax = contactBoxMax;
	
	// Compute the displacement and use it to sweep the box
	const Vector displacement = gpGlobals->frametime * m_vPreUpdateVelocity;
	...
	ComputeAABBContactsWithBrushEntity( this, contacts, pClip, iClipCount, WorldSpaceCenter(), contactBoxMin, contactBoxMax, pWorld );
										 ...
										 char const* context )
{
	...
	const Vector lookAheadVector = lookAheadTime * m_vPreUpdateVelocity;
	...
	for( int sample = 0; sample < numSamples; distance += stepSize, ++sample )
	{
		...
		ComputeAABBContactsWithBrushEntity( this, contacts, pClip, iClipCount, center, contactBoxMin + offset, contactBoxMax + offset, pWorld );
...
PaintPowerState CPortal_Player::ActivateSpeedPower( PaintPowerInfo_t& speedInfo )
{
	...
	SetMaxSpeed( clamp( tangentialVelocity.Length(), NORMAL_SPEED, sv_speed_paint_max.GetFloat() ) );
...
PaintPowerState CPortal_Player::UseSpeedPower( PaintPowerInfo_t& speedInfo )
{
	...
	fldv = (flWishCos < 0.0f) ? ( flWishCos * fabs( flSpeed - NORMAL_SPEED ) ) : (flSpeedFactor * fldv);

	float flDefaultNewMaxSpeed;
	if ( (GetFlags() & FL_DUCKING) || m_Local.m_bDucked )
	{
		flDefaultNewMaxSpeed = clamp( MaxSpeed() + fldv, NORMAL_SPEED * 0.33, sv_speed_paint_max.GetFloat() * 0.33 );
	}
	else
	{
		flDefaultNewMaxSpeed = clamp( MaxSpeed() + fldv, NORMAL_SPEED, sv_speed_paint_max.GetFloat() );
	}
	...
	SetMaxSpeed( GetLocalVelocity().IsZero() ? NORMAL_SPEED : flNewMaxSpeed );
											  ...
											  float flWishDirSpeed ) const
{
	...
	else if( flWishCos > 0.f && flWishCos < 0.3f )
	{
		...
		// their total speed.
		flNewMaxSpeed = MAX( flNormalSpeed, flWishDirSpeed + 0.3f * (flSpeed - flWishDirSpeed) );
	}
	else
	{
...
bool CPortal_Player::IsTryingToSuperJump( const PaintPowerInfo_t* pInfo ) const
{
	...
	const Vector& velocity = m_vPreUpdateVelocity;
...
{	
	...
	const bool bIsActuallyDucked = (m_Local.m_bDucking || GetFlags() & FL_DUCKING);
	const bool bDuckIsHeld = (m_nButtons & IN_DUCK) != 0;
	const bool bIsCrouching = !m_Local.m_bInDuckJump ? bIsActuallyDucked : bDuckIsHeld;
	...
	if( bShouldJump && !bIsCrouching )
	{
		...
		//m_flAirInputScale = BOUNCE_PAINT_INPUT_DAMPING; 
		...
		{
			m_Local.m_flJumpTime = (GAMEMOVEMENT_JUMP_TIME * 2); // The duck jump time should be longer than it normally is
			m_Local.m_bInDuckJump = true;
		}

		// Compute velocity in the normal direction
		Vector velocity = m_vPreUpdateVelocity;
			  ...
			  bounce_reflect_restitution.GetFloat() * fabs( normalSpeed ) >= bounce_paint_min_speed.GetFloat() )
		{
			...
			velocity = ComputeBouncePostVelocityReflection( velocity, info.m_SurfaceNormal, ABS_UP );
		}
		else
		{
			// Otherwise, perform a jump
			velocity = ComputeBouncePostVelocityNoReflect( velocity, info.m_SurfaceNormal, ABS_UP );
		}
		...
		if ( sv_debug_bounce_reflection.GetBool() )
		{
			...
			NDebugOverlay::VertArrow( info.m_ContactPoint, info.m_ContactPoint + 50.f * ABS_UP, 2, 0, 0, 255, 64, true, sv_debug_bounce_reflection_time.GetFloat() );
		...
		if( GetInAirState() == ON_GROUND && bounceSurfaceIsGround ) 
		{
			...
			Vector vMaxLateralVel = vLateralVelocity.Normalized() * (NORMAL_SPEED + NORMAL_SPEED * sv_bounce_paint_forward_velocity_bonus.GetFloat());
...
PaintPowerState CPortal_Player::ActivateBouncePower( PaintPowerInfo_t& info )
{
	...
	PaintPowerState state = bUsedPower ? DEACTIVATING_PAINT_POWER : ACTIVE_PAINT_POWER;
...
{
	m_fBouncedTime = gpGlobals->curtime + fTimeOffset;
}
...
void CPortal_Player::UpdateAirInputScaleFadeIn()
{
	...
	{
		//const bool doneDeactivating = IsDeactivatingPower( GetPaintPower( SPEED_POWER ) ) && MaxSpeed() == NORMAL_SPEED && GetGroundEntity() != NULL;
...
{
	const InAirState state = m_InAirState;

	// The player jumped
	if( JumpedThisFrame() )
		m_InAirState = IN_AIR_JUMPED;

	// The player bounced
	if( BouncedThisFrame() )
		m_InAirState = IN_AIR_BOUNCED;
...
void CPortal_Player::CachePaintPowerChoiceResults( const PaintPowerChoiceResultArray& choiceInfo )
{
	...
	else
	{
		for( int i = 0; i < count; ++i )
		{
			...
			m_CachedPaintPowerChoiceResults[i].wasValid = wasValid;
			m_CachedPaintPowerChoiceResults[i].wasIgnored = choiceInfo[i].bWasIgnored;
			if( wasValid )
			{
				m_CachedPaintPowerChoiceResults[i].surfaceEntity = choiceInfo[i].pPaintPower->m_HandleToOther;
				m_CachedPaintPowerChoiceResults[i].surfaceNormal = choiceInfo[i].pPaintPower->m_SurfaceNormal;
			}
		}
	}
}
