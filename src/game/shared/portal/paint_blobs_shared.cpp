...
const int BLOB_TRACE_DYNAMIC_MASK = CONTENTS_SOLID | CONTENTS_WINDOW | CONTENTS_HITBOX | CONTENTS_MOVEABLE | CONTENTS_MONSTER | CONTENTS_DEBRIS;
...
int CBasePaintBlob::CheckCollisionThroughPortal(BlobCollisionRecord *pCollisions, int maxCollisions, const Vector &vecEndPos)
{
	...
	int writeIndex = 0;
	CProp_Portal* pLastHitPortal = NULL;
	const int lastSegIndex = segmentCount - 1;
	for (int i = 0; i < segmentCount; ++i)
	{
		CProp_Portal* pEndPortal = traceSegments[i].pSegmentEndPortal;
		if (pEndPortal != NULL &&
...
void CBasePaintBlob::ResolveCollision(bool& bDeleted, const BlobCollisionRecord& collision, Vector& targetVelocity, float deltaTime)
{
		...
		//If the blob went through a portal then teleport it out of the portal
	{
		if (m_MoveState != PAINT_BLOB_STREAK_MOVE)
		{
			CProp_Portal* pInPortal = assert_cast<CProp_Portal*>(collision.trace.m_pEnt);
...
void CBasePaintBlob::PaintBlobMoveThroughPortal(float flDeltaTime, CProp_Portal *pInPortal, const Vector &vecStartPos, const Vector &vecTransformedEndPos)
{
...
bool CBasePaintBlob::PaintBlobStreakPaint(const Vector &vecBlobStartPos)
{
	...
	CProp_Portal *pInPortal = UTIL_Portal_TraceRay(blobRay, contentMask, &traceFilter, &trace);
