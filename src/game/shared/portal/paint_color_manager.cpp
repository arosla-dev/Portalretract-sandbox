...
ConVar portal_paint_color( "portal_paint_color", "224 224 224 255", FCVAR_REPLICATED, "Color for portal paint");
...
PaintPowerType MapColorToPower( const Color& color )
{
	...
	if ( color.a() < 80 ) // Retract: Changing from 0 to 80 (extremely tiny paint splotches can activate a power, and we don't want that)
		return NO_POWER;
...
PaintPowerType MapColorToPower( const CUtlVector<Color>& colors )
{
	// Find the hit count of each power for the colors in the array
	unsigned powerCount[PAINT_POWER_TYPE_COUNT_PLUS_NO_POWER] = { 0 };
	PaintPowerType power;
	for ( int i = 0; i < colors.Count(); ++i )
	{
		power = MapColorToPower( colors[i] );
		AssertMsg( power >= 0 && power < PAINT_POWER_TYPE_COUNT_PLUS_NO_POWER, "Invalid power type! This may cause out-of-bounds indexing!" );
		++powerCount[ power ];
	}
