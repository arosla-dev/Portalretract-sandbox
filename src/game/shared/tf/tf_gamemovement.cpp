...
public:
	...
	virtual const Vector& GetPlayerViewOffsetTf(bool ducked) const;
...
const Vector& CTFGameMovement::GetPlayerViewOffsetTf(bool ducked) const
{
...
bool CTFGameMovement::CheckJumpButton()
{
	...
	if ((player->m_Local.m_bDucking && (player->GetFlags() & FL_DUCKING)) || (player->m_Local.m_flDuckJumpTime > 0)) 
		return false;
...
void CTFGameMovement::ResetGetPointContentsCache()
{
...
int CTFGameMovement::GetPointContentsCached(const Vector& point, int slot)
{
...
void CTFGameMovement::CategorizePosition(void)
{
	...
	if (trace.plane.normal.z < flStandableZ)
	{
		...
		TracePlayerBBoxForGround(m_pTraceListData, vecStartPos, vecEndPos, GetPlayerMins(),
			GetPlayerMaxs(), PlayerSolidMask(), pFilter, trace, flStandableZ, true, &m_nTraceCount);
