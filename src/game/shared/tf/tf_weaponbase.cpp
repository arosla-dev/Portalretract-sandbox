...
void CTFWeaponBase::CreateMuzzleFlashEffects( C_BaseEntity *pAttachEnt, int nIndex )
{
	...
	CBasePlayer *pLocalPlayer = C_BasePlayer::GetLocalPlayer( GET_ACTIVE_SPLITSCREEN_SLOT() );
...
int	CTFWeaponBase::InternalDrawModel( int flags, const RenderableInstance_t &instance )
{
	...
	bool bNotViewModel = ( (pOwner && !pOwner->IsLocalPlayer()) || pOwner->ShouldDrawLocalPlayer() );
...
void CTFWeaponBase::ProcessMuzzleFlashEvent( void )
{
	...
	bool bDrawMuzzleFlashOnViewModel = ( pOwner->IsLocalPlayer() && !pOwner->ShouldDrawLocalPlayer() ) ||
